<!-- 载入时调用接口调用用户表、用户信息表和角色，实现数据的展示。在编辑和删除时应该调用接口将修改的信息传递到后端进行数据库的更新。性别，学校，角色应该用选择器进行选择，具体的数据应从数据库中取出 -->
<template>
<div>
  <el-table
    :data="tableData"
    ...
    methods: {
     ...
     //更新
      ...
      //this.$set(tableName,talbeIndex,data)
        this.$set(this.tableData,this.editForm.index,{name:this.editForm.name,account:this.editForm.account,sex:this.editForm.sex,role:this.editForm.role,school:this.editForm.school});
        this.editFormVisible = false;
      }
